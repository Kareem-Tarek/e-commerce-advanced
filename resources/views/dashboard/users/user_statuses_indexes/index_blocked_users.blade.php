                        ...
                        @if(auth()->user()->user_type == "admin")
                          <td class="text-center" style="@if($blocked_user->user_type == "admin" && $blocked_user->id != auth()->user()->id) background-color: lightgrey; @endif">                              
                            ...
                            @else
                              ...
                              <button class="btn btn-danger btn-md p-1 text-white" onclick="return confirm('Are you sure that you want to delete - {{ $blocked_user->name }}?');" type="submit" title="{{'Delete '."- ($blocked_user->name)"}}"><i class="fa-solid fa-trash dashboard-admin-icon-action"></i> Delete </button>
                            @endif
                            {!! Form::close() !!}
                          </td>
                        @endif
