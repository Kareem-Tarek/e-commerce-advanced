                        ...
                        @if(auth()->user()->user_type == "admin")
                            <td class="text-center" style="@if($user->user_type == "admin" && $user->id != auth()->user()->id) background-color: lightgrey; @endif">                              
                              ...
                              @else
                                ...
                                <button class="btn btn-danger btn-md @if(auth()->user()->user_type == "admin" && $user->id == auth()->user()->id) d-none @endif p-1 text-white" onclick="return confirm('Are you sure that you want to delete - {{ $user->name }}?');" type="submit" title="{{'Delete '."- ($user->name)"}}"><i class="fa-solid fa-trash dashboard-admin-icon-action"></i> Delete </button>
                              @endif
                              {!! Form::close() !!}
                          </td>
                        @endif
