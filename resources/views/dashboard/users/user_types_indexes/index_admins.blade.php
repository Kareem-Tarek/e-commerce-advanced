        ...
        <p class="card-description">
          ...
          {{-- Add class <code>.table-striped</code> --}}
            <span class="bg-secondary px-2 py-1 text-light rounded">
                Results (<span class="fw-bold">{{ $admins_count }}</span>)
            </span>
        </p>
        <div class="table-responsive mt-2">
          <table class="table table-striped table-bordered border border-secondary @if($admins_count == 0) d-none @endif">
            <thead>
            ...
            <tbody>
                @forelse($admins as $admin)
                    <tr>
                        ...
                        {{-- <td class="fw-bold text-primary">{{ $admin->product_id }}</td> --}}

                        <td class="@if($admin->avatar == null) text-center @endif">
                        ...
                        <td>{{ ucfirst($admin->username) }}</td>

                        <td>{{ ucfirst($admin->name) }}</td>
                        
                        <td>{{ $admin->email }}</td>

                        <td>{{ ucfirst($admin->user_type) }}</td>

                        <td>
                          @if($admin->status == "active")
                            <span class="bg-success text-white px-2 py-1 rounded">{{ ucfirst($admin->status) }}</span>
                          @elseif($admin->status == "inactive")
                            <span class="bg-warning text-dark px-2 py-1 rounded">{{ ucfirst($admin->status) }}</span>
                          @else($admin->status == "blocked")
                            <span class="bg-danger text-white px-2 py-1 rounded">{{ ucfirst($admin->status) }}</span>
                          @endif
                        </td>

                        <td class="@if($admin->phone == null) text-center @endif">
                          {{ $admin->phone ?? '—' }}
                        </td>

                        <td class="@if($admin->address == null) text-center @endif">
                          {{ $admin->address ?? '—' }}
                        </td>

                        <td class="@if($admin->bio == null) text-center @endif">
                          {{ Illuminate\Support\Str::limit($admin->bio, '30', '...') ?? '—' }}
                        </td>

                        <td class="@if($admin->gender == null) text-center @endif">
                          {{ $admin->gender ?? '—' }}
                        </td>

                        <td class="@if($admin->dob == null) text-center @endif">
                          {{ $admin->dob ?? '—' }}
                        </td>

                        @if(auth()->user()->user_type == "admin")
                          <td class="text-center" style="@if($admin->user_type == "admin" && $admin->id != auth()->user()->id) background-color: lightgrey; @endif">                              
                            {!! Form::open([
                            ...
                            @if($admin->user_type == "admin" && $admin->id != auth()->user()->id)
                              —
                              {{-- <a style="display: none;" href="{{route('users.edit', $admin->id)}}" class="btn btn-primary btn-md p-1 text-white" type="button" title="{{'Edit '."- ($admin->name)"}}"><i class="fas fa-edit dashboard-admin-icon-action"></i> Edit</a> --}}
